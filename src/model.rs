...
fn draw_model_inner_matrix(
    ...
    // using glam here to generate the correct matrix, then casting it to the naive
    // row-major implementation
    let model_to_proj_matrix = projection_matrix.mul_mat4(&model_matrix);
    let model_to_proj_matrix = unsafe {
        std::mem::transmute::<[f32; 16], crate::math::Mat4>(
            model_to_proj_matrix.transpose().to_cols_array(),
        )
    };

    fn camera_to_screen_space(
        width: usize,
        height: usize,
        mut v: crate::math::Vec4,
    ) -> crate::math::Vec3 {
        v.x /= v.w;
        v.y /= v.w;
        v.z /= v.w;
